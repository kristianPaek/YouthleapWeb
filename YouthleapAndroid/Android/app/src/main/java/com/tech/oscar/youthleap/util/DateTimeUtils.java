...
public class DateTimeUtils {
	...
	public static String convertDateTimeToString(Date date) {
		...
		try {
			...
			if (nowTime < dateTime)
				return YouthLeapApp.getContext().getResources().getString(R.string.checkin_time_minus);

			long timeInterval = ((new Date()).getTime() - date.getTime() )/ 1000;

			if (timeInterval < 1*MINUTE) {
				formattedTime = String.format(YouthLeapApp.getContext().getResources().getString(
			...
			e.printStackTrace();
			formattedTime = YouthLeapApp.getContext().getResources().getString(R.string.checkin_time_minus);
		}
